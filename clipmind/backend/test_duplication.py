...
def run_tests():
    ...
    try:
        ...
        def mock_llm_call_fallback(system_prompt, user_prompt, response_json=True):
    ...
    try:
        ...
        with patch("analyzer.LLMResilienceManager") as MockResilience:
            ...
            inst.call.side_effect = lambda system_prompt, user_prompt, **kwargs: json.dumps({"candidates": scout_cand}) if "researcher" in system_prompt else "{}"
            MockResilience.return_value = inst
    ...
    # --- Test Case 5: Job Cache Reuse ---
    # Concurrent/duplicate URL submissions must return the same job ID
    try:
        import asyncio
        from main import analyze_video, AnalyzeRequest
        
        # Setup mock request
        req = AnalyzeRequest(url="https://www.youtube.com/watch?v=dQw4w9WgXcQ", gemini_api_key="real_key_abc")
        ...
        with patch("main.check_ffmpeg", return_value=True):
            ...
            async def run_reqs():
                ...
                res1 = await analyze_video(req, bg)
                res2 = await analyze_video(req, bg)
                return res1, res2
                
            res1, res2 = asyncio.run(run_reqs())
            
            if res1["job_id"] == res2["job_id"]:
                record("Job Cache: duplicate URL submissions reuse active job ID", PASS, f"Reused job ID: {res1['job_id']}")
            else:
                record("Job Cache: duplicate URL submissions reuse active job ID", FAIL, f"Job 1: {res1['job_id']}, Job 2: {res2['job_id']}")
    except Exception as e:
