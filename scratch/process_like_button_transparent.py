...
def process_video_transparent():
    print("=== Transparent Watermark Removal Processor ===")
    ...
    # 3. Clean each frame and convert to RGBA transparent
    print("Cleaning watermarks and converting background to transparent...")
    ...
    # 4. Reassemble frames into a 60 FPS transparent WebM (VP9) video
    print("Reassembling cleaned frames into transparent WebM (VP9)...")
    assemble_cmd = [
        "ffmpeg", "-y",
        "-r", "60",
    ...
    # Remove the MP4 version if it exists under assets/animations so the WebM is used
    mp4_version = os.path.join(output_dir, "like_button.mp4")
    if os.path.exists(mp4_version):
        os.remove(mp4_version)
        print("Removed conflicting MP4 version from assets/animations")
